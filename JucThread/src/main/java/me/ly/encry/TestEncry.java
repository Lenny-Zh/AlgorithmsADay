...
public class TestEncry {
    ...
    public static void main1(String[] args) {
        ...
        try {
            //
            byte[] encrypt = encryptOrdecrypt(true, str.getBytes("UTF-8"), psw, null, AESType.AES_128, "AES/ECB/PKCS5Padding");
            System.out.println(new String(encrypt));
            byte[] bytes2 = encryptOrdecrypt(false, encrypt, psw, null, AESType.AES_128, "AES/ECB/PKCS5Padding");
            System.out.println(new String(bytes2));

            byte[] bytes = encryptAES(str, psw);
            byte[] encodeBase64 = Base64.getEncoder().encode(bytes);
            String encodeBase64Str = new String(encodeBase64);
            byte[] bytes1 = decryptAESS(Base64.getDecoder().decode(encodeBase64Str), psw);
